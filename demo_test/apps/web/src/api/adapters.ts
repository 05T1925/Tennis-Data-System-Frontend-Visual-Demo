...
import type { ResultDto, TaskDto, UserDto, VideoDto } from './dto';
...
const adaptCourtPoint = (point: ResultDto['heatmap_points'][number]): CourtPoint => ({
  x: point.x,
  y: point.y,
...
export function adaptResultDto(dto: ResultDto): AnalysisResult {
  const shots: ShotRecord[] = dto.shots.map((shot) => ({
    ...
    startPoint: adaptCourtPoint(shot.start_point),
    endPoint: adaptCourtPoint(shot.end_point),
    bouncePoint: shot.bounce_point ? adaptCourtPoint(shot.bounce_point) : undefined,
  ...
  return {
    ...
    rallies,
    points,
    heatmapPoints: dto.heatmap_points.map(adaptCourtPoint),
    createdAt: dto.created_at,
  };
}
